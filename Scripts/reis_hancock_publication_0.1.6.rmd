...
An overview of total records data is available in *Table 1.1: Record Data Overview*.

<br>

<center>

**Table 1.1: Record Data Overview**

```{r record_overview_tbl, echo=FALSE, message=FALSE, cache=TRUE}
library(kableExtra)
...
**In terms of individual employees, rather than records,** the number of unique workers are determined by using each unique combination of social security numbers (or unique identification codes if unavailable), titles, vocations, races (if available), zip codes, and company names. In total, there are 189 unique workers, 126, or 66.7% of whom have race disclosed, while 63, or 33.3% do not. An overview of unique workers data is available below in *Table 1.2: Unique Worker Data Overview*.

<br>

<center>

**Table 1.2: Unique Worker Data Overview**

```{r inds_overview_tbl, echo=FALSE, message=FALSE, cache=TRUE}
library(kableExtra)
...
**Figure 8: Descriptive Statistics of Hourly Wage v. Race**
...
**Figure 9.1: Descriptive Statistics of Hourly Wage v. Race by Company**
...
**Figure 10.2: Descriptive Statistics of Net Pay v. Race**
...
```{r choropleth_workers_output, message=FALSE, warning=FALSE, echo=FALSE, fig.width=8.25, fig.height=7, cache=TRUE, eval = TRUE}

tmap_mode(mode = "view")

tm_shape(county_shapes) +
  tm_fill(col = "WORKERS",
          title = "Workers", 
          style = "cont",
          alpha = 0.75, 
          popup.vars = c("State:" = "STATE", 
                         "Workers:" = "WORKERS")) +
  tm_borders(col = "White",
             lwd = 1.6) +
  tm_text(text = "NAME", size = .75) +
  tm_layout(title = "Workforce Density", 
            title.size = 1.15, 
...
*Click on a county to see its state and the number of workers there.*
...
names(cnt_rc)[c(1,11)] <- c("NAME", "STATEFP")

cnt_rc_sf <- inner_join(cnt_sf, cnt_rc, by = c("NAME", "STATEFP"))
cnt_rc_sf <- cnt_rc_sf[, c(5, 1:4, 6:27)]
```
...
lf_rc_cnt <- tm_shape(cnt_rc_sf) +
  ...
  tm_borders(col = "White",
             lwd = 1.6) +
  tm_text(text = "NAME", size = .6) +
  tm_layout(title = "Racial Composition by County", 
            title.size = 1.15, 
            title.position = c("LEFT", "TOP"), 
            main.title.position = c("LEFT", "TOP"),
            legend.title.size = 1,
...
*Click on a county to view the total workers, proportion of minority workers, and breakdown of workers by race.*
...
zip_race_sf <- zip_race_sf[, c(10, 1:9, 11:20)]

zip_race_sf$Main <- zip_race_sf$City
zip_race_sf <- zip_race_sf[, c(21, 1:20)]
...
```{r choropleth_zip_race, message=FALSE, warning=FALSE, echo=FALSE, fig.width=8.25, fig.height=7, cache=TRUE, eval=TRUE}

tmap_mode(mode = "view")

my_tm <- tm_shape(zip_race_sf) +
  ...
  tm_borders(col = "White",
             lwd = 1.6) +
  tm_text(text = "City", size = .6) +
  tm_layout(title = "Racial Composition", 
            title.size = 1.15, 
            title.position = c("LEFT", "TOP"), 
            main.title.position = c("LEFT", "TOP"),
            legend.title.size = 1,
...
my_tm <- tmap_leaflet(my_tm)
my_tm %>% setView(lng = -76.1474, lat = 43.0481, zoom = 9)
```

*Click on an area to see the zip code, county, total workers, proportion of minority workers, and breakdown by race.*
...
### Summary: Mean Total Net by County
