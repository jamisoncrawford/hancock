...
**Duplicate records were detected and recently removed.** Therefore, visualizations and tables herein, which are likely accurate but must undergo review, may differ from their accompanying interpretations. The analayst anticipates the most affected records involve net pay and, notably, the number of unique records has decreased from 1,045 to 927. 
...
**Figure 8.3: Frequency of Hourly Wages Earned by Race & Contractor**
...
net <- master %>% 
  ...
  rename("Name" = name, "Title" = title, "Race" = race, "Net" = net)
...
net_sum <- net_sum %>%
  ...
  summarize(Payments = n(),
            `Total` = sum(Net)) %>%
  ungroup() %>%
  mutate(`Total Earnings (%)` = percent(Total / sum(Total)),
         `Race Earnings ($)` = dollar(Total)) %>%
...
net_con_sum <- net %>%
  ...
  ungroup() %>%
  mutate(`Total Earnings (%)` = percent(`Total` / sum(`Total`)),
         `Company Earnings ($)` = dollar(`Total`)) %>%
  arrange(Name, desc(Payments))
...
net_con_sum <- net_con_sum %>%
  group_by(Name) %>%
  mutate(`Company Earnings (%)` = percent(Total/sum(Total))) %>%
  select(Name:Payments, `Company Earnings ($)`, `Company Earnings (%)`, `Total Earnings (%)`) %>%
  bind_cols(as_data_frame(con_table)) %>%
  select(-Contractor, - Race1) %>%
  rename("Company Workers" = `Unique Workers`, 
         "Company Workers (%)" = Percentage, 
         "Contractor" = Name)
```
...
```{r echo=FALSE, message=FALSE, fig.width=12, fig.height=7}
ggplot(net, aes(x = Race, y = Net)) +
  geom_boxplot(fill = "#FF9900", alpha = 0.35, outlier.shape = 15, outlier.size = 2) +
  geom_jitter(height = .3, width = .2, fill = "#003399", color = "#003399", alpha = 0.15, size = 2) +
  coord_flip() + 
  ylab("Hourly Pay (USD)") +
  theme_light()
```
...
ggplot(net_con_sum_viz, aes(x = reorder(Name, Total), y = Total, fill = Race)) +
  geom_bar(stat = "identity", alpha = 0.6) +
  coord_flip() +
  xlab("Contractor") +
  scale_fill_brewer(palette = "Set2") +
...
ggplot(net[net$Race == "White", ], aes(x = Name, y = Net, color = Race)) +
  geom_jitter(alpha = 0.4, width = 0.175) +
  geom_jitter(data = net[net$Race != "White", ], size = 3, width = 0.175, alpha = 0.9, aes(x = Name, y = Net, color = Race)) +
  xlab("Contractor") +
  scale_color_brewer(palette = "Set2") +
  theme_classic() +
  theme(legend.position = "bottom") +
...
ggplot(net, aes(x = Race, y = Net)) +
  geom_boxplot(alpha = 0.75, color = "lightgray", outlier.shape = 15, outlier.size = 3) +
  geom_jitter(alpha = 0.25, height = .2, width = .3, size = 2,
              aes(color = Race)) +
  scale_color_brewer(palette = "Set2") +
  xlab("Race") +
  coord_flip() +
...
**Figure 13: Total Workers v. Top 10 Cities & All Other by County**

```{r echo=FALSE, message=FALSE, warning=FALSE, fig.width=12, fig.height=7}
library(ggplot2)

ggplot(city_10, aes(x = reorder(City, `Total Workers`), 
                   y = `Total Workers`,
                   fill = County)) +
  geom_bar(stat = "identity") +
  coord_flip() +
...
race_trunc <- county_race[11:29, ] %>%
  summarize(County = "All Other",
            Race = "White", 
            Total = sum(Total),
            `Race (%)` = sum(`Race (%)`),
            `Workforce (%)` = sum(`Workforce (%)`))

county_race_tbl <- county_race[1:10, ] %>%
...
**Table 14: Top Counties of Worker Origin for All Races**
...
county_race_viz <- county_race[c(1:10, 11, 20), 1:3] %>%
  bind_rows(race_trunc[, 1:3]) %>%
  arrange(desc(Race), desc(Total), County)

county_race_viz[1, "Total"] <- 33
county_race_viz[1, "County"] <- "Other"
...
city_race_tbl <- city_race %>%
  mutate(`City (%)` = percent(`City (%)`),
         `Race (%)` = percent(`Race (%)`),
         `County (%)` = percent(`County (%)`)) %>%
  rename("Race in City (%)" = `City (%)`,
         "Total Workers (%)" = `County (%)`) %>%
  select(1:3, 5, 4, 6)
```
...
county_net_tbl <- county_net %>%
  ...
  mutate(`Mean Net per Worker` = dollar(`Total Net ($)` / `Total Workers`),
         `Mean Net per Check` = dollar(`Total Net ($)` / `Total Checks`),
...
county_net_raw <- county_net %>%
  ...
  mutate(`Worker Difference` = `Mean Net per Worker` - 3302.185,
         ...
         `Worker (%)` = percent((`Mean Net per Worker` / 3302.185)),
         `Check (%)` = percent((`Mean Net per Check` / 838.2468)))
```
