---
title: 'Hancock Renovations: Importing Whites, Exporting Wealth'
...
setwd("~/Projects/REIS/Maps")
tigris_cache_dir("~/Projects/REIS/Maps")
options(scipen = 999,
...
```{r concept_art, cache=TRUE, echo=FALSE, fig.cap="*Concept rendering for the passenger terminal at Hancock International Airport. Source: [Syracuse.com](https://www.syracuse.com/business-news/index.ssf/2017/10/heres_what_syracuse_airport_will_look_like_after_49m_renovation_photos.html)*", out.width = '90%'}
...
**Analysis of employment data on 2018 Hancock International Airport renovations**, conducted to inform the Racial Equity Impact Statement (REIS), reveals insights into racial disparities ranging from negligible to severe, as well as remarkable insights in terms of where workers' disposable income goes. 927 unique payment records were analyzed. 703 (75.8%) contain race data and 926 (99.9%) contain location data among 189 unique workers across 6 companies.

<br>

## Findings: Race

**Where race is disclosed, the vast majority of workers are White** (86.5%), comprising 109 workers compared to 17 minority workers. African American workers represent the majority of minority workers, while least represented is Hispanic workers, of whom there was only 1 in the entire workforce.

**White workers worked more weeks than they comprise in the workforce, proportionately.** Despite comprising 86.5% of the total workforce, White workers account for 88.3% of all pay periods, while all minority races worked marginally lower numbers of weeks compared to their proportion of the workforce. Parsed by company, only *Stone Bridge Iron & Steel, Inc.* reversed this trend. Notably, the 1 Hispanic worker accounted for only 2 of the 703 payments recorded.

**There were no minority workers in specialist or leadership positions.** While 14.3% of White workers assumed the position of "Foreman" or some variation, and 5.6% assumed the position of "Operator" (typically the most lucrative position aside from "Foreman"), minority workers only comprised "Journeyman" and "Apprentice" positions. 

**No systematic racial disparities in hourly pay are present.** Most hourly pay is determined by the position or title of a worker, e.g. "Foreman", "Journeyman", "Apprentice", etc.

**White workers received the vast majority of total net earnings.** Most disposable income, or gross less deductions, is received by White workers, who earned $609.4 K, or 91.1% of total net earnings, which is 4.6% greater than their proportion of the workforce. All minority workers received total net earnings that were proportionately less than their representation in the workforce, with African Americans receiving the most ($36.3 K) and Hispanics the least ($966).

<br>

## Findings: Location

**Where location is disclosed, only one third of workers live in Onondaga County.** Onondaga had the greatest amount of workers, 66 or 35.1%, while Oswego had the second greatest, 52 or 27.7%.

**One in five workers live in Syracuse, while remaining workers come from 80 other cities and towns.** While Syracuse, NY is home to 36 workers, or 19.1%, Oswego, NY and Fulton, NY are home to the second and third most workers, at 18 and 17, or 9.6% and 9.0%, respectively.

**Most minority workers reside in Onondaga County.** Specifically, 8 of 9 African Americans, 2 of 4 Native Americans, 2 of 3 workers of Two or More Races, and the sole Hispanic Worker reside there. Over 75% of White workers live outside of Onondaga.

**Out of $850 K in net earnings, over 3 of every 4 dollars left Onondaga County.** The 66 workers residing in the county, comprising 35.1% of the workforce, received only 25.6% of total earnings - remarkably less relative to their proportion of the total workforce - at $217.9 K.

**Oswego workers earned significantly than Onondaga workers, proportionately.** Compared to Onondaga's 66 workers, comprising 35.1% of the workforce and receiving 25.6% of total net earnings, Oswego's 52 workers, comprising 27.7% of the total workforce, earned 23.8% of total net earnings. The average Oswego worker earned a total net of $3.9 K, 17.9% more than the average Onondaga worker.

**Tioga County, PA had the third greatest total net earnings - and went to only 5 workers.** Comprised of 4 Journeymen and 1 Apprentice, or 2.7% of the total workforce, Tioga County received total net earnings of $49.8 K. The average worker in Tioga County, PA received $9.9 K in total net earnings, or 201.8% more than the average total net of an Onondaga worker.

**Even greater average total net earnings are found throughout NY and PA.** For example, average total net earnings per worker in Cortland, NY, exceeded $10.7 K, or 225.3% more than the average Onondaga worker, while 1 worker in Schoharie County, NY, single-handedly ranked Schoharie as 12th in total net earnings out of 25 counties, earning $19.4 K, or 486.5% greater than the average Onondaga worker. There are several other staggering examples.

**More than 7 out of every 10 paychecks went to workers outside Onondaga County.** Onondaga workers accounted for only 260 pay periods, or 28.9% of nearly 900 paychecks. Moreover, staggering differences in average net payments, like total net earnings, were discovered.

**Workers who live farther away from Syracuse tend to make more money.** There is a weak to moderate correlation ($r = 0.29$) between the straight-line distance from Syracuse, NY and total net earnings. In ranges of 25-mile increments, while total workforce representation decreases, average net earnings per worker increase substantially, with the exception of 125- to 150-mile range.
...
**Notably, 100% of employees were male,** according to demographic records provided. Therefore, gender disparities are neither possible to determine, approximate, nor impute.

<br>

## On Demographic Data

```{r data_overview, echo=FALSE, message=FALSE, cache=TRUE}
library(scales)
library(dplyr)
...
```{r individuals_overview, echo=FALSE, message=FALSE, cache=TRUE}
library(dplyr)
library(scales)
...
overview <- data_frame("Level" = c("Total Records", "Total Records"),
...
ind_overview <- data_frame("Level" = c("Unique Workers", "Unique Workers"),
                           "Status" = c("Available", "Missing"),
                           "Race" = c(126, 63),
                           "Race (%)" = c("66.7%", "33.3%"),
                           "Zip Code" = c(188, 1),
...
mrg_ovw <- bind_rows(overview, ind_overview)
```
...
**In terms of individual employees, rather than records,** the number of unique workers are determined by using each unique combination of social security numbers (or unique identification codes if unavailable), titles, vocations, races (if available), zip codes, and company names. In total, there are 189 unique workers, 126, or 66.7% of whom have race disclosed, while 63, or 33.3% do not. An overview of total records and records on unique workers is available below in *Table 1: Data Overview*.
...
```{r overview_tbl, echo=FALSE, message=FALSE, cache=TRUE}
library(kableExtra)

mrg_ovw <- mrg_ovw %>% select(-Level)

kable(mrg_ovw) %>%
  kable_styling(bootstrap_options = c("striped", "hover", "responsive"),
                full_width = TRUE) %>%
  group_rows(group_label = "Unique Payment Records", start_row = 1, end_row = 2) %>%
  group_rows(group_label = "Unique Workers", start_row = 3, end_row = 4)
```
...
```{r ind_race_overview, message=FALSE, echo=FALSE, cache=TRUE}
library(dplyr)
...
```{r race_count_table, message=FALSE, echo=FALSE, cache=TRUE}
library(kableExtra)
library(flextable)
library(scales)
...
```{r race_count_viz, echo=FALSE, message=FALSE, fig.width=12, fig.height=7, cache=TRUE}
library(ggplot2)
...
**Further breaking down unique worker race by contractors reveals additional findings** about workforce diversity, or lack thereof, within the companies themselves. For example, *Table 3: Unique Workers by Race & Company* reveals that all 9 workers in *Patricia Electric, Inc.* are White, while *Edward Schalk & Son, Inc.*, had more than twice the unique workers, 24 of whom were White (92.3%) and the remaining 2 were African American (7.7%). *Longhouse Construction LLC* had a similar racial makeup albeit a somewhat differing proportion of White to African American workers, with 1 of 7 unique workers identified as African American (14.3%) and the remaining 6 workers identified as White (85.7%). 

**Notably, the lowest proportion of unique White workers** (69.2%) were hired by *Stone Bridge Iron & Steel, Inc.*, which also accounts for most unique Native American workers among all companies (75%), with 3 of 4 of the workforce's Native Americans identified within the company, comprising 23.1% of its workers. *Stone Bridge* also had, on record, the only Hispanic worker among all companies, comprising 7.7% of company workers. *Quality Structures, Inc.* had the greatest diversity in terms of distinct races. To wit, White, African American, Two or More Races, and Native American workers were among its ranks, though the vast majority, 61 of its 71 workers, were White (85.9%). *Figure 3: Unique Workers v. Race by Company* illustrates these proportions among each company.
...
```{r contractor_race_table, echo=FALSE, message=FALSE, cache=TRUE}
library(flextable)
library(scales)
...
**Figure 3: Unique Workers v. Race by Company**

```{r contractor_race_viz, echo=FALSE, message=FALSE, fig.width=12, fig.height=7, cache=TRUE}
library(ggplot2)
library(Cairo)
...
```{r periods_by_race, echo=FALSE, message=FALSE, cache=TRUE}
library(dplyr)
...
```{r periods_by_race_tbl, echo=FALSE, message=FALSE, cache=TRUE}
kable(week_table) %>%
  kable_styling(bootstrap_options = c("striped", 
                                      "hover", 
                                      "responsive"),
...
```{r periods_by_race_viz, echo=FALSE, message=FALSE, fig.width=12, fig.height=7, cache=TRUE}
...
```{r periods_race_con, echo=FALSE, message=FALSE, warning=FALSE, cache=TRUE}
library(dplyr)
library(scales)
...
**Breaking down total periods worked by race and contractor reinforces proportional disparities.** *Table 5: Total Weeks Worked by Race & Company* depicts the total count and proportion of working periods by race among each company. *Quality Structures, Inc.* appears to have the greatest disparities, with Whites comprising 85.9% of company workers but taking home 449, or 88.2%, of the 509 paychecks distributed, while African Americans comprised 8.5% of workers but only received 7.1% of paychecks distributed within the company. The 4.2% of workers of Two or More Races in *Quality Structures, Inc.* took home a total of 14 paychecks (2.8%) while, atypically, the 1 Native American among its 71 workers (1.4%) received 1.8% of all distributed paychecks.

**Racial disparities in weeks worked are smaller among other contractors.** *Longhouse Construction LLC* exhibited small disparities, with an 85.7% of Whites working 86.4% of its total work weeks and 14.3% of African Americans working 13.6% of total work weeks. More equitably, the 92.3% of White workers in *Edward Schalk & Son, Inc.* received 73 of 79 total payments (92.4%) while the remaining 7.6% of paychecks were distributed to its 2 African American workers (7.7%), or 7.6% of all payment periods, indicating a practically negligible proportional disparity relative to the company's racial composition. Notably, *Stone Bridge Iron & Steel, Inc.* was the only organization which reversed this trend, with 69.2% of Whites working 60.6% of total work periods, while the company's 3 Native Americans employees, comprising 23.1% of company workers, received 11 payments, or 33.3% of work periods. The only exception being the 1 Hispanic worker, comprising 7.7% of the company's unique workers yet taking home 2 paychecks, or 6.1%, total company payments. These disparities are further illustrated in *Figure 5: Total Weeks Worked v. Race by Company*.
...
```{r periods_race_con_tbl, echo=FALSE, message=FALSE, cache=TRUE}
week_con_table2 <- week_con_table %>%
  as_data_frame() %>%
  select(-Company)

kable(week_con_table2) %>%
  kable_styling(bootstrap_options = c("striped", 
                                      "hover", 
                                      "responsive"),
                full_width = TRUE) %>%
...
```{r periods_race_con_viz, echo=FALSE, message=FALSE, fig.width=12, fig.height=7, cache=TRUE}
...
```{r title_by_race, echo=FALSE, message=FALSE, cache=TRUE}
library(dplyr)
library(scales)
library(stringr)
...
```{r titles_by_race_tbl, echo=FALSE, message=FALSE, cache=TRUE}
title_agg_table2 <- title_agg_table %>%
  as_data_frame() %>%
  select(-Race)

kable(title_agg_table2) %>%
  kable_styling(bootstrap_options = c("striped", 
                                      "hover", 
                                      "responsive"),
                full_width = TRUE) %>%
...
```{r titles_by_race_viz, echo=FALSE, message=FALSE, fig.width=12, fig.height=7, cache=TRUE}
...
```{r title_by_race_con, echo=FALSE, message=FALSE, cache=TRUE}
library(dplyr)
library(scales)
library(stringr)
...
```{r title_by_race_con_tbl, echo=FALSE, message=FALSE, cache=TRUE}
titles_con_agg_table2 <- titles_con_agg_table %>%
  as_data_frame() %>%
  select(-Company)

kable(titles_con_agg_table2) %>%
  kable_styling(bootstrap_options = c("striped", 
                                      "hover", 
                                      "responsive"),
                full_width = TRUE) %>%
...
**Figure 7: Titles v. Total Workers by Race & Company**

```{r title_by_race_con_viz, echo=FALSE, message=FALSE, fig.width=12, fig.height=7, cache=TRUE}
...
```{r rate_by_race, echo=FALSE, message=FALSE, cache=TRUE}
library(dplyr)
library(scales)
...
```{r rate_by_race_tbl, echo=FALSE, message=FALSE, cache=TRUE}
kable(hourly_sum) %>%
  kable_styling(bootstrap_options = c("striped", 
                                      "hover", 
                                      "responsive"),
...
**Figure 8: Summary Statistics of Hourly Wage v. Race**

```{r rate_by_race_viz, echo=FALSE, message=FALSE, fig.width=12, fig.height=7, cache=TRUE}
set.seed(1)
...
```{r rate_by_race_con_tbl, echo=FALSE, message=FALSE, cache=TRUE}
hourly_con_sum2 <- hourly_con_sum %>%
  as_data_frame() %>%
  select(-Company)

kable(hourly_con_sum2) %>%
  kable_styling(bootstrap_options = c("striped", 
                                      "hover", 
                                      "responsive"),
                full_width = TRUE) %>%
...
**Figure 9.1: Summary Statistics of Hourly Wage v. Race by Company**

```{r rate_by_race_con_sum, echo=FALSE, message=FALSE, fig.width=12, fig.height=7, cache=TRUE}
set.seed(1)
...
**Alternate visualizations help further illustrate these conclusions**. *Figure 9.2: Summary Statistics of Hourly Wage v. Title & Race by Company* supports the conclusion that hourly wage disparities are principally due to differences in titles. Moreover, the distribution of hourly wages conditioned on race and contractor is illustrated in *Figure 9.3: Frequency of Unique Hourly Wages Earned by Race & Company*, which depicts a stacked trellis histogram representing the frequency of pay grades for each unique worker. Each bar represents a $1 increment in hourly rates. In other words, the taller the bar, the greater the number of unique workers fell within the $1 range of hourly pay represented along the horizontal axis.

<br>

<center>

**Figure 9.2: Summary Statistics of Hourly Wage v. Title & Race by Company**

```{r rate_by_race_con_title_sum, echo=FALSE, message=FALSE, fig.width=12, fig.height=8, cache=TRUE}
...
**Figure 9.3: Frequency of Unique Hourly Wages Earned by Race & Company**

```{r rate_by_race_con_title_frq, echo=FALSE, message=FALSE, fig.width=12, fig.height=8, cache=TRUE}
library(ggplot2)
...
```{r net_race_con_tbl, echo=FALSE, message=FALSE, warning=FALSE, cache=TRUE}
library(dplyr)
library(scales)
...
```{r net_by_race_tbl, echo=FALSE, message=FALSE, cache=TRUE}
kable(net_sum) %>%
  kable_styling(bootstrap_options = c("striped", 
                                      "hover", 
                                      "responsive"),
...
```{r net_by_race_viz, echo=FALSE, message=FALSE, fig.width=12, fig.height=8, cache=TRUE}
library(ggplot2)
library(scales)
...
### Stats: Net Pay by Race

```{r net_by_race_sum, echo=FALSE, message=FALSE, cache=TRUE}
net_stat <- net %>%
  filter(Race != "Hispanic") %>%
  group_by(Race) %>%
...
**Similar to hourly wage, maximum net payment for White workers is significantly higher than minority workers.** According to *Table 10.2: Summary Statistics on Individual Net Payments by Race*, while maximum take home pay for African Americans, Native Americans, and workers of Two or More Races were $1,601, $1,006, and $864, respectively, while the maximum amount received by a White worker was significantly higher, at $2,478. Note that the single Hispanic earned the same net income for 2 periods worked, $483, which by virtue of worker count and payment frequency, is both the minimum and maximum net pay. Note that the Hispanic worker has been excluded from the summary statistics below. 

**The disparity between average net earnings of White and minority workers are just as extreme.** That is, African American workers' average net earnings per pay period are $807, Native Americans earn an average of $652, and workers of Two or More Races earn an average of $638. In contrast, White workers, on average, receive $1,002 per net payment - a markedly higher sum. Median values deviate marginally from mean values, at an average absolute value of $24.43. These findings are better illustrated in *Figure 10.2: Summary Statistics of Net Pay v. Race*, and aid in the interpretation of boxplots is described earlier in "Section 5.8: Hourly Pay by Race". 
...
```{r net_by_race_sum_tbl, echo=FALSE, message=FALSE, cache=TRUE}
kable(net_stat) %>%
  kable_styling(bootstrap_options = c("striped", 
                                      "hover", 
                                      "responsive"),
...
**Figure 10.2: Summary Statistics of Net Pay v. Race**

```{r net_by_race_sum_viz, echo=FALSE, message=FALSE, fig.width=12, fig.height=7, cache=TRUE}
net_hisp_rm <- net %>%
  filter(Race != "Hispanic")
...
```{r net_by_race_con_table, echo=FALSE, message=FALSE, cache=TRUE}
net_con_sum2 <- net_con_sum %>%
  as_data_frame() %>%
  select(-Company)
...
**Figure 11.1: Net Earnings v. Company by Race**

```{r net_by_race_con_viz, echo=FALSE, message=FALSE, fig.width=12, fig.height=8, cache=TRUE}
library(ggplot2)
...
**Figure 11.2: Individual Net Earnings v. Company by Race**

```{r net_by_race_con_scatviz, echo=FALSE, message=FALSE, fig.width=12, fig.height=8, cache=TRUE}
library(ggplot2)
set.seed(3)
...
```{r net_race_con_sum, echo=FALSE, message=FALSE, cache=TRUE}
net_con_stat <- net %>%
  group_by(Company, Race) %>%
  summarize(Minimum = min(Net),
            Average = mean(Net),
...
```{r net_race_con_sum_tbl, echo=FALSE, message=FALSE, cache=TRUE}
net_con_stat2 <- net_con_stat %>%
  as_data_frame() %>%
  select(-Company)
...
**Figure 11.3: Summary Statistics of Net Pay v. Race by Company**

```{r net_race_con_sum_viz, echo=FALSE, message=FALSE, fig.width=15, fig.height=10, cache=TRUE}
library(ggplot2)
...
**Alternate visualizations again help illustrate the distribution of net earnings conditioned on race and contractor.** *Figure 11.4: Frequency & Distribution of Net Pay by Race & Company* depicts a stacked trellis histogram depicting the frequency of weekly net payments and their distributions by race and conditioned on contractor. Each bar represents a $25 increment in net payments, while taller bars indicate higher frequencies of net payments within each $25 range.  

<br>

<center>

**Figure 11.4: Frequency & Distribution of Net Pay by Race & Company**

```{r net_race_con_sum_frq, echo=FALSE, message=FALSE, fig.width=15, fig.height=10, cache=TRUE}
library(ggplot2)
...
```{r geo_all, echo=FALSE, message=FALSE, eval=FALSE, cache=TRUE}
geo <- master %>% filter(!is.na(zip))
geo %>% filter(!is.na(net)) %>% nrow()
geo %>% select(name:rate, zip) %>% unique()
```

**Though the initial impetus behind analyzing employment data was driven by racial disparities**, exploratory analysis revealed another significant discovery among records. To wit, most workers hired among most contractors were from neither Syracuse nor Onondaga County. In fact, a number of workers traveled to Syracuse from across the state, while some of the highest paid workers were imported from northern counties in Pennsylvania. 
...
```{r workers_by_county, echo=FALSE, message=FALSE, cache=TRUE}
county <- master %>%
  filter(!is.na(zip)) %>%
...
```{r workers_by_county_map, echo=FALSE, message=FALSE, warning=FALSE, include=FALSE, cache=TRUE}
library(sf)
library(tmap)
library(tigris)
library(leaflet)
...
for (i in 1:nrow(county_chor)){
  county_chor$County[i] <- paste(county_chor$County[i], "County", sep = " ")
}

shapes <- tigris::counties(state = c(36, 42), 
                           resolution = '20m',
                           class = "sf")
shapes <- fortify(shapes)

county_chor <- county_chor %>% rename("county_name" = County,
                                      "state" = State,
                                      "workers" = `Total Workers`) %>%
  left_join(counties, by = c("county_name", "state")) %>%
  mutate(full_fips = paste(state_fips, county_fips, sep = "")) %>%
  select(-`Total Workers (%)`, -county_name, -state, -fips_class, 
         -CSA, -CBSA, -population, -full_fips) %>%
  rename("COUNTYFP" = county_fips,
         "STATEFP" = state_fips,
         "WORKERS" = `workers`)

county_shapes <- inner_join(shapes, county_chor, by = c("STATEFP", "COUNTYFP"))
county_shapes$STATE <- NA
county_shapes <- county_shapes[, c(6, 20, 1:5, 7:19)]

for (i in 1:nrow(county_shapes)){
  if (county_shapes$STATEFP[i] == "36"){
    county_shapes$STATE[i] <- "New York"
  } else if (county_shapes$STATEFP[i] == "42"){
    county_shapes$STATE[i] <- "Pennsylvania"
  }
}

county_shapes$wrk_prc <- NA

for (i in 1:nrow(county_shapes)){
  county_shapes$wrk_prc[i] <- percent(county_shapes$WORKERS[i] / 188, accuracy = .1)
}
```
...
```{r workers_by_county_tbl, echo=FALSE, message=FALSE, cache=TRUE}
kable(county) %>%
  kable_styling(bootstrap_options = c("striped", 
                                      "hover", 
                                      "condensed",
...
**Figure 12.1: Total Workers v. County by State**

```{r workers_by_county_viz, echo=FALSE, message=FALSE, warning=FALSE, fig.width=12, fig.height=7, cache=TRUE}
library(ggplot2)
...
**Figure 12.2: Workforce Density by County**

```{r chor_workers_output, message=FALSE, warning=FALSE, echo=FALSE, fig.width=8.25, fig.height=7, cache=TRUE, eval = TRUE}

tmap_mode(mode = "view")

tm_workers_cnt <- tm_shape(county_shapes) +
  tm_fill(col = "WORKERS",
          title = "Workers", 
          style = "cont",
          alpha = 0.75, 
          popup.vars = c("Workers:" = "WORKERS",
                         "Proportion:" = "wrk_prc")) +
  tm_borders(col = "White",
             lwd = 1.6) +
  tm_text(text = "NAME", size = .75) +
  tm_layout(main.title.position = c("LEFT", "TOP"),
            legend.title.size = 1,
            legend.position = c("RIGHT", "BOTTOM"),
            frame = FALSE, saturation = -3)

lf_workers_cnt <- tmap_leaflet(tm_workers_cnt)
lf_workers_cnt %>% setView(lng = -76.1474, lat = 43.0481, zoom = 7)
```

*Click on a county to see its number and proportion of workers.*
...
```{r workers_by_city, echo=FALSE, message=FALSE, cache=TRUE}
city <- master %>%
  filter(!is.na(zip)) %>%
  select(name:rate, zip:state) %>%
  unique() %>%
...
```{r workers_by_city_tbl, echo=FALSE, message=FALSE, cache=TRUE}
kable(city_10) %>%
  kable_styling(bootstrap_options = c("striped", 
                                      "hover", 
                                      "condensed",
...
**Figure 13.1: Total Workers v. Top 10 Cities by County**

```{r top_10_city_bar, echo=FALSE, message=FALSE, warning=FALSE, fig.width=12, fig.height=7, cache=TRUE}
library(ggplot2)
...
```{r race_county, echo=FALSE, message=FALSE, cache=TRUE}
county_race <- master %>%
  filter(!is.na(zip),
         !is.na(race)) %>%
  select(name:rate, zip:state) %>%
...
**Most minority workers, where race data are disclosed, reside in Onondaga County.** According to *Table 14.1: Top Counties of Worker Origin for All Races*, 88.9% of the 9 African American workers in the total workforce, reside in Onondaga County, while 1 traveled from Oneida County. Half of the 4 Native American workers reside in Onondaga County, with the remaining half traveling from Cattaraugus County. Among workers of Two or More Races, 2 of 3 reside in Onondaga County, while 1 traveled from Cayuga County. Lastly, the single Hispanic worker also resides in Onondaga County. In total, 13 of the 17 minority workers among the total workforce (76.5%) originated in Onondaga County.

**Over 75% of White workers reside outside Onondaga County.** That is, only 26, or 24.1%, of White workers reside within Onondaga County, while the greatest number of White workers, 34 or 31.5%, traveled from Oswego County. The third highest concentration of White workers are the 8 workers residing in Madison County, comprising 7.4% of Whites and 6.4% of the total workforce. The 40 remaining White workers, comprising 37% of total White workers and 32% of the total workforce, came from counties other than Onondaga, Oswego, and Madison. These findings are illustrated in *Figure 14.1: Top Counties of Worker Origin for All Races* and *Figure 14.2: Racial Composition of Workforce by County*. A full list of workers by race and county is available in the *Appendix* under *Table 14.2*. 
...
```{r race_county_tbl, echo=FALSE, message=FALSE, cache=TRUE}
...
**Note:** Although there may appear to be discrepancies between *Table 14.1: Top Counties of Worker Origin for All Races* and *Figure 14: Top Counties of Worker Origin for All Races*, the above table parses counties by and is exhaustive of race, while the below figure combines counties to avoid double-counting unique workers.

<center>

<br>

**Figure 14.1: Top Counties of Worker Origin for All Races**

```{r county_race_bar, echo=FALSE, message=FALSE, warning=FALSE, fig.width=12, fig.height=7, cache=TRUE}
library(ggplot2)
...
```{r county_race_prep, echo=FALSE, message=FALSE, warning=FALSE, include=FALSE, cache=TRUE}
library(tidyr)

cnt_rc <- county_race %>%
  select(-`Race (%)`, -`Workforce (%)`) %>%
  spread(key = Race, value = Total) %>%
  group_by(County, State, `African American`, Hispanic, `Native American`, `Two or More Races`, White) %>%
  summarize_all(funs(.[which(!is.na(.))])) %>%
  as_data_frame() %>%
  mutate(Total = NA,
         min_tot = NA,
         min_prc = NA,
...
for (i in 1:nrow(cnt_rc)){
  cnt_rc$Total[i] <- sum(cnt_rc[i, 3:7], na.rm = TRUE)
  cnt_rc$min_tot[i] <- sum(cnt_rc[i, 3:6], na.rm = TRUE)
  cnt_rc$min_prc[i] <- round(sum(cnt_rc[i, 3:6], na.rm = TRUE) / cnt_rc$Total[i], digits = 2) * 100
  cnt_rc$`Minority (%)`[i] <- percent((cnt_rc$min_prc[i] / 100), accuracy = 1)}

for (i in 1:nrow(cnt_rc)){
if (cnt_rc$State[i] == "NY"){
    cnt_rc$st_fips[i] <- "36"
  } else if (cnt_rc$State[i] == "PA"){
    cnt_rc$st_fips[i] <- "42"
  }
}

cnt_rc[is.na(cnt_rc)] <- 0

cnt_sf <- tigris::counties(state = c("36", "42"), 
                           resolution = "20m", 
                           class = "sf")

cnt_sf <- fortify(cnt_sf)
names(cnt_rc)[c(1,12)] <- c("NAME", "STATEFP")

cnt_rc_sf <- inner_join(cnt_sf, cnt_rc, by = c("NAME", "STATEFP"))
cnt_rc_sf <- cnt_rc_sf[, c(6, 1:5, 7:28)]
```

**Figure 14.2: Racial Composition of Workforce by County**

```{r chor_cnty_rc_map, message=FALSE, warning=FALSE, echo=FALSE, fig.width=8.25, fig.height=7, cache=TRUE, eval=TRUE}
library(tmap)
library(leaflet)
library(tmaptools)
...
lf_rc_cnt <- tm_shape(cnt_rc_sf) +
  tm_fill(col = "min_prc",
          title = "% Minority", 
          style = "cont",
          alpha = 0.75, 
          popup.vars = c("Workers:" = "Total",
                         "Minorities:" = "min_tot",
                         "Proportion:" = "Minority (%)")) +
  tm_borders(col = "White",
             lwd = 1.6) +
...
lf_rc_cnt <- tmap_leaflet(lf_rc_cnt)
lf_rc_cnt %>% setView(lng = -76.1474, lat = 43.0481, zoom = 7)
```

*Click on a county to view the total workers, minority workers, and proportion of minority workers.*
...
```{r race_onondaga, echo=FALSE, message=FALSE, cache=TRUE}
city_race <- master %>%
  filter(!is.na(zip),
         !is.na(race),
         county == "Onondaga") %>%
...
```{r race_onondaga_tbl, echo=FALSE, message=FALSE, cache=TRUE}
city_race_tbl2 <- city_race_tbl %>%
  as_data_frame() %>%
  select(-Race)

kable(city_race_tbl2) %>%
  kable_styling(bootstrap_options = c("striped", 
                                      "hover", 
                                      "condensed",
                                      "responsive"),
...
**Figure 15.1: Location of Onondaga County Workers by Race**

```{r race_onondaga_viz, echo=FALSE, message=FALSE, warning=FALSE, fig.width=12, fig.height=7, cache=TRUE}
library(ggplot2)
...
```{r zipcode_choropleth_prep, echo=FALSE, message=FALSE, warning=FALSE, include=FALSE, cache=TRUE}
library(tidyr)
...
zip_race <- master %>%
  ...
  select(name:rate, zip:state) %>%
  unique() %>%
  group_by(city, zip, county, state, race) %>%
  summarize(Total = n()) %>%
  rename("Race" = race, "City" = city, "Zip Code" = zip, "County" = county, "State" = state) %>%
  arrange(Race, desc(Total), County) %>%
  as_data_frame() %>%
  group_by(Race) %>%
  mutate(`Race (%)` = percent(Total / sum(Total))) %>%
  as_data_frame() %>%
  mutate(`Workforce (%)` = percent(Total / sum(Total))) %>%
  select(-`Race (%)`, - `Workforce (%)`) %>%
  spread(key = "Race", value = "Total") %>%
  group_by(City, `Zip Code`, County, State, `African American`, `Hispanic`, 
           `Native American`, `Two or More Races`, `White`) %>%
  summarize_all(funs(.[which(!is.na(.))])) %>%
  as_data_frame() %>%
  mutate(min_tot = NA,
         `Minority (%)` = NA)

for (i in 1:nrow(zip_race)){
  zip_race$min_tot[i] <- sum(zip_race[i, 5:8], na.rm = TRUE)
  zip_race$`Minority (%)`[i] <- percent(sum(zip_race[i, 5:8], 
                                            na.rm = TRUE) / sum(zip_race[i, 5:9], 
                                                                na.rm = TRUE), accuracy = .1)
}

zip_race[is.na(zip_race)] <- 0
zip_race$Workers <- NA

for(i in 1:nrow(zip_race)){
  zip_race$Workers[i] <- rowSums(zip_race[i, 5:9], na.rm = TRUE)
}

zip_sf <- tigris::zctas(state = c("36", "42"), 
                        starts_with = c("12", "13", "14", "16", "17"), 
                        class = "sf")
zip_sf <- fortify(zip_sf)
names(zip_race)[2] <- "ZCTA5CE10"

zip_race_sf <- inner_join(zip_sf, zip_race, by = "ZCTA5CE10")
zip_race_sf <- zip_race_sf[, c(10, 1:9, 11:21)]

zip_race_sf$Main <- zip_race_sf$City
zip_race_sf <- zip_race_sf[, c(22, 1:21)]
zip_race_sf[c(48, 50:53, 55:57), "City"] <- ""
zip_race_sf[35, "City"] <- "Salina"
zip_race_sf[54, "City"] <- "Solvay & Van Buren"
zip_race_sf[35, "Main"] <- "Salina"
zip_race_sf[54, "Main"] <- "Solvay & Van Buren"

zip_race_sf$minor_perc <- zip_race_sf$`Minority (%)`
zip_race_sf$minor_perc <- str_replace_all(zip_race_sf$minor_perc, 
                                          pattern = "%", 
                                          replacement = "")
zip_race_sf$minor_perc <- as.numeric(zip_race_sf$minor_perc)


```

**Figure 15.2: Racial Composition by Zip Codes**

```{r chor_zip_race, message=FALSE, warning=FALSE, echo=FALSE, fig.width=8.25, fig.height=7, cache=TRUE, eval=TRUE}
library(tmap)
library(tmaptools)
library(leaflet)
...
tm_zip_race <- tm_shape(zip_race_sf) +
  tm_fill(col = "minor_perc",
          title = "% Minority", 
          style = "cont",
          alpha = 0.75, 
          popup.vars = c("Total Workers:" = "Workers",
                         "Minority (%):" = "Minority (%)",
                         "County:" = "County",
                         "Zip:" = "ZCTA5CE10")) +
  tm_borders(col = "White",
             lwd = 1.6) +
  tm_text(text = "City", size = .75) +
  tm_layout(main.title.position = c("LEFT", "TOP"),
            legend.title.size = 1,
            legend.position = c("RIGHT", "BOTTOM"),
            frame = FALSE, saturation = -3)

lf_zip_race <- tmap_leaflet(tm_zip_race)
lf_zip_race %>% setView(lng = -76.1474, lat = 43.0481, zoom = 9)
```

*Click on an area to see the zip code, county, total workers, and proportion of minority workers.*
...
```{r net_by_county_all, echo=FALSE, message=FALSE, cache=TRUE}
county_net <- master %>%
  filter(!is.na(zip),
         !is.na(net)) %>%
...
```{r worker_net_by_county_tbl, echo=FALSE, message=FALSE, cache=TRUE}
...
```{r net_by_county_viz, echo=FALSE, message=FALSE, warning=FALSE, fig.width=12, fig.height=7, cache=TRUE}
library(ggplot2)
...
### Stats: Mean Total Net by County
...
```{r total_by_county_sumstats, echo=FALSE, message=FALSE, cache=TRUE}
...
```{r total_by_county_boxplot, echo=FALSE, message=FALSE, warning=FALSE, fig.width=12, fig.height=7, cache=TRUE}
library(ggplot2)
...
```{r check_net_by_county_tbl, echo=FALSE, message=FALSE, cache=TRUE}
...
```{r net_payment_by_county_calc, echo=FALSE, message=FALSE, warning=FALSE, cache=TRUE}
net_payments <- master %>%
  filter(!is.na(zip), !is.na(net)) %>%
  group_by(county) %>%
...
```{r net_by_county_boxplot, echo=FALSE, message=FALSE, warning=FALSE, fig.width=12, fig.height=7, cache=TRUE}
library(ggplot2)
...
**Figure 16.4: Total Earnings by County**

```{r cont_net_chor_prep, echo=FALSE, message=FALSE, warning=FALSE, include=FALSE, cache=TRUE}
cnt_net_prep <- county_net_raw
cnt_net_prep$STATEFP <- NA

for (i in 1:nrow(cnt_net_prep)){
  if (cnt_net_prep$State[i] == "NY"){
    cnt_net_prep$STATEFP[i] <- "36"
  } else if (cnt_net_prep$State[i] == "PA"){
    cnt_net_prep$STATEFP[i] <- "42"
  }
}

cnt_net_sf <- tigris::counties(state = c("36", "42"), 
                               resolution = "20m", 
                               class = "sf")

cnt_net_sf <- fortify(cnt_net_sf)
names(cnt_net_prep)[1] <- "NAME"
cnt_net_sf <- inner_join(cnt_net_sf, 
                         cnt_net_prep, 
                         by = c("NAME", "STATEFP"))

cnt_net_sf$dollars <- dollar(cnt_net_sf$`Total Net`)
cnt_net_sf$perc <- percent(cnt_net_sf$`Percent (%)`)
cnt_net_sf$`Total Net` <- round(cnt_net_sf$`Total Net` / 1000, digits = 1)
cnt_net_sf <- cnt_net_sf[, c(6, 1:5, 7:35)]

cnt_net_sf$wrk <- NA
cnt_net_sf$chk <- NA

for (i in 1:nrow(cnt_net_sf)){
  cnt_net_sf$wrk[i] <- paste(cnt_net_sf$`Total Workers`[i], " (", 
                             cnt_net_sf$`Total Workers (%)`[i], ")", sep = "")
  cnt_net_sf$chk[i] <- paste(cnt_net_sf$`Total Checks`[i], " (", 
                             percent(cnt_net_sf$`Checks (%)`[i], accuracy = .1), ")", sep = "")
}
```

```{r net_cnt_leaf, message=FALSE, warning=FALSE, echo=FALSE, fig.width=8.25, fig.height=7, cache=TRUE, eval = TRUE}

tmap_mode(mode = "view")

tm_net_cnt <- tm_shape(cnt_net_sf) +
  tm_fill(col = "Total Net",
          title = "Net (K)", 
          style = "cont",
          alpha = 0.75, 
          popup.vars = c("Total Net:" = "dollars", 
                         "Total (%):" = "perc",
                         "Workers:" = "wrk",
                         "Checks:" = "chk")) +
  tm_borders(col = "White",
...
lf_net_cnt <- tmap_leaflet(tm_net_cnt)
lf_net_cnt %>% setView(lng = -76.1474, lat = 43.0481, zoom = 7)
```

*Click on a county to see its total net earnings, proportion of workforce earnings, workers, and pay periods.*
...
**As distance increases, total workers decrease but average weeks and average total earnings tend to increase.** As distance ranges increase from 25 to 50, 50 to 75, 75 to 100, and 100 to 125, total workers decrease, numbering 49, 12, 13, and 3, while average weeks per worker numbers 2.5, 7.3, 7.9, and 9, and average total net per worker amounts to $4.4 K, $8.3 K, $9 K, and $10.4 K, respectively. The exception being the furthest cohort, between 125 and 150 miles from Syracuse, which has the lowest values for weeks worked, average weeks per worker, total net earnings, and total net earnings per worker.
...
```{r distance_all, echo=FALSE, message=FALSE, warning=FALSE, cache=TRUE}
library(geosphere)
...
```{r dist_table, echo=FALSE, message=FALSE, cache=TRUE}
kable(dist_table) %>%
  kable_styling(bootstrap_options = c("striped", 
                                      "hover", 
                                      "condensed",
...
**Figure 17.1: Total Worker Net Earnings v. Distance from Syracuse**

```{r total_by_dist_scatter, echo=FALSE, message=FALSE, warning=FALSE, fig.width=12, fig.height=7, cache=TRUE}
...
**Figure 17.2: Total Worker Net Earnings & Distance from Syracuse**

```{r jitter_nets_prep, echo=FALSE, message=FALSE, warning=FALSE, include=FALSE, cache=TRUE}
dst_tots <- total_by_dist %>%
  select(name:race, total_earned:meeus_dist) %>%
  unique() %>%
  rename("ZCTA5CE10" = zip)
  

lf_zip_net <- inner_join(zip_sf, dst_tots, by = "ZCTA5CE10")
lf_zip_net$meeus_dist <- round(lf_zip_net$meeus_dist, digits = 1)
lf_zip_net$chk_prc <- NA
lf_zip_net$dis_mi <- NA
lf_zip_net$tot_prc <- NA
for (i in 1:nrow(lf_zip_net)){
  lf_zip_net$chk_prc[i] <- paste(lf_zip_net$total_chks[i], " (", 
                                 percent(lf_zip_net$total_chks[i] / 885, 
                                         accuracy = .11), ")", sep = "")
  lf_zip_net$dis_mi[i] <- paste(lf_zip_net$meeus_dist[i], "Miles", sep = " ")
  lf_zip_net$tot_prc[i] <- paste(dollar(lf_zip_net$total_earned[i]), " (", 
                                 percent(lf_zip_net$total_earned[i] / 839277.7, 
                                         accuracy = .11), ")", sep = "")
}

lf_zip_net <- lf_zip_net[, c(10, c(1:9, 11:28))]
lf_zip_net[which(is.na(lf_zip_net$race)), "race"] <- "Unspecified"

names(lf_zip_net)[17] <- "Net (K)"
for (i in 1:nrow(lf_zip_net)){
  lf_zip_net$`Net (K)`[i] <- lf_zip_net$`Net (K)`[i] / 1000
}

lf_zip_net$name_dist <- NA
for (i in 1:nrow(lf_zip_net)){
  lf_zip_net$name_dist[i] <- paste(lf_zip_net$city[i], lf_zip_net$dis_mi[i], sep = ", ")
}

lf_zip_net <- lf_zip_net[, c(29, 1:28)]
lf_zip_net[which(is.na(lf_zip_net$title)), "title"] <- "Unspecified"
```

```{r jitter_ind_nets, message=FALSE, warning=FALSE, echo=FALSE, fig.width=8.25, fig.height=7, cache=TRUE, eval = TRUE}

tmap_mode(mode = "view")

set.seed(1)

tm_zip_net <- tm_shape(lf_zip_net) +
  tm_symbols(size = "Net (K)",
             col = "Net (K)", 
             jitter = 0.4, 
             alpha = 0.7, 
             size.lim = c(0, 25),
             style = "cont",
             popup.vars = c("Total Net:" = "tot_prc",
                            "Total Checks:" = "chk_prc",
                            "Title:" = "title",
                            "Race:" = "race")) +
  tm_layout(main.title.position = c("LEFT", "TOP"),
            legend.title.size = 1,
...
lf_tots <- tmap_leaflet(tm_zip_net)
lf_tots %>% setView(lng = -76.1474, lat = 43.0481, zoom = 10)
```

*Each bubble represents one worker. Click on one to see his total earnings, weeks worked, title, and race.*
...
The following information is for the purposes of reproducible research for the present analysis. Steps for complete reproducibility, from raw data to the findings below, are provided in the [REIS repository](https://github.com/jamisoncrawford/REIS). Publication and tooling versions and dates are as follows:
...
```{r net_by_county_sumstats, echo=FALSE, message=FALSE, cache=TRUE}
