...
urls <- c("https://raw.githubusercontent.com/jamisoncrawford/REIS/master/Manual%20Scrapes/danforth_company_manual_scrape_1.1.csv",
          "https://raw.githubusercontent.com/jamisoncrawford/REIS/master/Manual%20Scrapes/environmental_services_manual_scrape_1.1.csv",
          "https://raw.githubusercontent.com/jamisoncrawford/REIS/master/Manual%20Scrapes/longhouse_manual_scrape_1.1.csv",
          "https://raw.githubusercontent.com/jamisoncrawford/REIS/master/Manual%20Scrapes/niagara_manual_scrape_1.1.csv",
          "https://raw.githubusercontent.com/jamisoncrawford/REIS/master/Manual%20Scrapes/patricia_manual_scrape_1.1.csv",
          "https://raw.githubusercontent.com/jamisoncrawford/REIS/master/Manual%20Scrapes/stone_bridge_manual_scrape_1.1.csv")
...
write_csv(danfo, "danforth_table_1.1.csv")                                     # Write to .csv

for(i in 1:nrow(danfo)){for (j in 1:col(danfo)){
  if (is.na(danfo[i, j])){danfo[i, j] <- "" }}}                                # NAs to empty cells

write_csv(danfo, "danforth_table_narm_1.1.csv"); rm(danfo, i, j)               # Write to .csv, narm; remove
...
envir <- envir %>%
  ...
  select(ssn:class, gross, net, ending, zip:longitude, pdf_pg:occur)           # Rearrange variables
...
write_csv(envir, "environmental_table_1.1.csv")                                # Write to .csv

for(i in 1:nrow(envir)){for (j in 1:col(envir)){
  if (is.na(envir[i, j])){envir[i, j] <- "" }}}                                # NAs to empty cells

write_csv(envir, "environmental_table_narm_1.1.csv"); rm(envir, i, j)          # Write to .csv, narm; remove
...
longh <- longh %>%
  mutate(zip = as.character(zip)) %>%                                          # Coerce class: zip
  left_join(y = zipcode, by = "zip") %>%                                       # Merge location data
  select(ssn:exempt, week_wg, net, ending, zip, city:longitude, pdf_pg:occur)  # Rearrange variables
...
write_csv(longh, "longhouse_1.1.csv")                                          # Write to .csv

for(i in 1:nrow(longh)){for (j in 1:col(longh)){
  if (is.na(longh[i, j])){longh[i, j] <- "" }}}                                # NAs to empty cells

write_csv(longh, "longhouse_narm_1.1.csv"); rm(longh, i, j)                    # Write to .csv, narm; remove
...
niagr <- niagr %>% 
  ...
  left_join(y = zipcode, by = "zip") %>%                                       # Merge location data
  select(ssn, wage, ending, zip, city:longitude, pdf_pg:occur, -net)           # Rearrange variables
...
niagr <- niagr[-(which(mdy(niagr$ending) == "2018-02-24")[7:12]), ]            # Remove duplicates
niagr <- niagr[-30, ]                                                          # Remove negative wage

write_csv(niagr, "niagara_1.1.csv")                                            # Write to .csv
write_csv(niagr, "niagara_narm_1.1.csv"); rm(niagr, i, j, k)                   # Write to .csv, narm; remove
...
write_csv(patri, "patricia_1.1.csv")                                           # Write to .csv
write_csv(patri, "patricia_narm_1.1.csv"); rm(patri)                           # Write to .csv, narm; remove
...
stone <- stone %>%
  mutate(zip = as.character(zip)) %>%                                          # Coerce class: zip
  left_join(y = zipcode, by = "zip") %>%                                       # Merge location data
  select(ssn:title, rate, week_wg, net, ending, zip:longitude, pdf_pg:occur)   # Rearrange variables

write_csv(stone, "stone_bridge_1.1.csv")                                       # Write to .csv
write_csv(stone, "stone_bridge_narm_1.1.csv"); rm(stone, zipcode)              # Write to .csv, narm; remove