...
quali$position <- NA
quali <- quali %>% select(-union, -eeo, -page, -string)
...
sands$state <- as.factor(sands$zip)
...
master <- master %>% select(name, ssn, title, position, sex:race, rate, net, ending, 
                            zip:state, longitude, latitude, pdf_no, pdf_pg, occur)

# WRITE TO CSV

write_csv(master, "hancock_master_table_1.0.csv")
